...
# Container/runtime preparation at import time (mirrors previous behavior)
if constants.isInContainer:
    ...
    if os.geteuid() == 0:  # pyright: ignore[reportAttributeAccessIssue,reportUnknownMemberType]
        if constants.uid == -1 or constants.gid == -1:
            logger.warning(
                f"Environment variable(s) DRPP_UID and/or DRPP_GID are/is not set. Manually ensure appropriate ownership of {constants.runtimeDirectory}"
            )
            ...
            if constants.noRuntimeDirChown:
                logger.warning(
                    f"Environment variable DRPP_NO_RUNTIME_DIR_CHOWN is set to true. Manually ensure appropriate ownership of {constants.runtimeDirectory}"
                )
            else:
...
from . import cache, config, discord, images, plex
...
def main() -> None:
    if not config.config["users"]:
        ...
        config.save()
    plexAlertListeners = [
        plex.PlexAlertListener(user["token"], server)
